...
async def log_docker_container_errors(state: GraphState, llm):
    print("\n **LOG DOCKER CONTAINER ERRORS AGENT **")
    error = None
    container_name = state["docker_container_name"]
    check_interval = 3  # Interval for checking logs (in seconds)
    ...
    try:
        ...
        while True:
            try:
                # Fetch the last 100 lines of logs from the container
                logs = container.logs(tail=100).decode("utf-8")
                print("\nContainer Logs:\n", logs)

                # Check for the presence of "error" in logs
                if "error" in logs.lower():
                    print("Error detected in logs!")

                    # Extract error details for structured error message
                    error_message = parse_error_from_logs(logs)
                    print(f"Error detected: {error_message}")

                    # Restart the container as a recovery action
                    ...
                    return None  # No error found, returning None

            except Exception as e:
                print(f"Failed to retrieve logs or process container: {e}")
                return ErrorMessage(type="Internal Code Error", details=str(e))

            # Wait for the specified interval before checking the logs again
            await asyncio.sleep(check_interval)

    except docker.errors.NotFound:
        print(f"Error: Container '{container_name}' not found.")
        return ErrorMessage(
            type="Dependency Error", details=f"Container '{container_name}' not found"
        )
    except Exception as e:
...
def parse_error_from_logs(logs: str) -> ErrorMessage:
    ...
    This is a simple example and can be customized to extract more detailed information from the logs.
    """
    # THIS NEED MODIFICATIONS???
    error_type = "Execution Error"  # Example type, can be customized based on logs
    error_details = logs  # For now, using the entire logs as details. You can refine this to be more specific.

    # If you want to extract more structured information, you can add parsing logic here.
    return ErrorMessage(
        type=error_type,
        details=error_details,
        # Additional fields like `file`, `line`, and `code_reference` can be parsed and added here.
    )
